...
module.exports = cds.service.impl(
    srv => {
      ...
      //  srv.after('READ', 'DepartmentGoal', goalDepTargetAchievedCal);
        ...
        srv.after('READ', "UserLogin", _UserLoginCheck);
    }
)

async function _UserLoginCheck(req, res) {
    let request = req;
    req.User = new cds.User("Hari");
    if(req.length > 1) {
        res.error(422, "Precondition/Filters are mandatory");
    }
